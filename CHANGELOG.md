...
- **Safe URI decoding**: Wrapped `decodeURIComponent()` in HTTP server route parsing with `safeDecodeURIComponent()` to prevent `URIError` crashes from malformed percent-encoded URL segments
- **Signal handler leak**: Moved signal handler registration outside the server start Promise and added cleanup on startup failure (`EADDRINUSE` etc.) to prevent leaked `SIGINT`/`SIGTERM` handlers

### Changed

- 2 new tests (total: 275)
