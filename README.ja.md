...
- **プロンプトインジェクション対策 (v0.14.0+)**: `handoff_load` が保存内容を「信頼できないコンテンツ」マーカーで囲んで返す — 警告バナー、偽造できないワンタイム境界トークン、本文中のバッククォート連続より長いコードフェンス
- **送信元メタデータ (v0.14.0+)**: `from_project` / `spawner_dispatch_id` / `sender_agent_id` のオプションフィールドで「誰が送ったか」を記録。特定のエコシステムに縛られない設計
